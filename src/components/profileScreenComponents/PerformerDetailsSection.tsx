...
const PerformerDetailsSection = ({performer}: PerformerDetailsSectionProps) => {
    ...
    const getGradeOrGraduationYear = ({grade, graduationYear, studyType}: Performer) => {
        const gradeYear = graduationYear ? `${t('GRADUATED_IN')}: ${graduationYear}` : t(`GRADE_${grade}`);
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-8">
            <div className="flex items-start gap-6">
                ...
                <div className="flex-1">
                    ...
                    <div>
                        {performer.age && (<h3 className="text-gray-600">{performer.age} {t('YEARS_OLD')}</h3>)}
                    </div>
                    <div>
                        {performer.height && (<h3 className="text-gray-600">{performer.height} {t('CM')}</h3>)}
                    </div>
                    <div className="flex items-center gap-2 text-gray-500">
                        <GraduationCap className="flex-shrink-0 w-4 h-4"/>
                        <span>{t(performer.department)} - {getGradeOrGraduationYear(performer)}</span>
                    </div>
